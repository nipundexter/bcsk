...
import { getPage, getSettings, renderMarkdown } from "@/lib/content";
import { CLASS_LEVELS } from "@/lib/constants";

export default async function HomePage() {
  const { lang, t } = await getDict();
  const [hero, welcome, stats, specialCourses, teachers, news] = await Promise.all([
    getPage("home-hero", lang),
    getPage("about-us", lang),
    getSettings(["stat_total_students", "stat_total_classes", "stat_special_courses", "stat_teachers_staff"]),
    db.course.findMany({ where: { type: "SPECIAL", active: true }, orderBy: { displayOrder: "asc" }, take: 6 }),
    db.teacherProfile.findMany({ where: { isPublic: true }, orderBy: { displayOrder: "asc" }, include: { user: true }, take: 4 }),
    db.eventNews.findMany({ where: { published: true }, orderBy: { date: "desc" }, take: 3 }),
  ]);
          ...
          <div className="grid lg:grid-cols-2 gap-8 items-center px-6 sm:px-12 py-12 lg:py-16">
            <div className="relative z-10">
              ...
              <div
                ...
                dangerouslySetInnerHTML={{ __html: renderMarkdown(hero?.content ?? "") }}
              />
      ...
      <section className="mx-auto max-w-7xl px-4 mt-20 grid lg:grid-cols-2 gap-10 items-center">
        <div className="relative">
          ...
          <div
            ...
            dangerouslySetInnerHTML={{ __html: renderMarkdown((welcome?.content ?? "").split("###")[0]) }}
          />
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
          {teachers.map((tp, i) => (
            <div key={tp.id} className="text-center">
              ...
              <h3 className="mt-3 font-bold text-sm text-ink">{tp.user.name}</h3>
        ...
        <div className="grid sm:grid-cols-3 gap-5">
          {news.map((n) => (
            <Link key={n.id} href={`/events/news/${n.id}`} className="group bg-white border border-line rounded-2xl p-6 hover:shadow-md transition-shadow">
              ...
              <time className="block mt-3 text-xs text-ink-soft" dateTime={n.date.toISOString()}>
                {n.date.toLocaleDateString(lang === "bn" ? "bn-BD" : lang === "ko" ? "ko-KR" : "en-US", { year: "numeric", month: "long", day: "numeric" })}
              </time>
            </Link>
          ))}
        </div>
