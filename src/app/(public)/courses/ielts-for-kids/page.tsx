...
export default async function IeltsPage() {
  const [course, fee, { slugs, studentUserId }] = await Promise.all([
    db.course.findUnique({
      where: { slug: "ielts-for-kids" },
      include: {
    ...
    db.feeConfig.findUnique({ where: { key: "ielts_for_kids" } }),
    getEnrolledCourseSlugs(),
  ]);
  const enrolled = slugs.has("ielts-for-kids");
