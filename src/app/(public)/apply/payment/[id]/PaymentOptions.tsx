...
import { createCardOrder, submitBankTransfer, devSimulateCardSuccess, type BankState } from "./actions";
...
export function PaymentOptions({
  ...
  async function payByCard() {
    ...
    setCardError(null);
    try {
      const { orderId } = await createCardOrder(applicationId, bcsk);
      ...
      await payment.requestPayment({
        ...
        failUrl: `${window.location.origin}/apply/payment/${applicationId}?error=card-declined${bcsk ? "&bcsk=1" : ""}`,
    ...
    <div>
      ...
      {method === "CARD" ? (
        <div className="bg-white border border-line rounded-2xl p-6">
          ...
          {devMode && (
            <button
              onClick={() => devSimulateCardSuccess(applicationId, bcsk)}
              className="mt-3 w-full border-2 border-dashed border-line hover:border-sky text-ink-soft hover:text-sky font-bold rounded-lg px-6 py-2.5 text-xs transition-colors"
            >
              DEV: simulate successful card payment
            </button>
          )}
        </div>
      ) : (
        <div className="bg-white border border-line rounded-2xl p-6">
          ...
          <form action={bankAction} className="mt-5 space-y-4">
            ...
            <input type="hidden" name="bcsk" value={bcsk ? "1" : "0"} />
